...
import net.kartikverma.childcare.model.Review;
...
//        Verify the parent owns this booking
        if(!booking.getParent().getId().equals(parent.getId())) {
            throw new ResourceNotfoundException("You can only review your own bookings");
        }

//        Only complete bookings can be reviewed
        if(!booking.getStatus().equals(BookingStatus.COMPLETED)) {
            throw new ResourceNotfoundException("You can only review completed bookings");
        }

//        Check review doesn't already exist for this booking
        if(reviewRepository.findByBookingId(booking.getId()).isPresent()) {
            throw new ResourceNotfoundException("You have already reviewed this booking");
        }
